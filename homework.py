...
class CaloriesCalculator(Calculator):
    ...
    def get_calories_remained(self):
        ...
        balance = self.limit - today_stats
    ...
    def get_week_stats(self):
        balanc_for_7_days = super().get_week_stats()
        return f'Поглащено за последние 7 дней {balanc_for_7_days}: кКал.'
...
class CashCalculator(Calculator):
    ...
    def get_today_cash_remained(self, currency='rub'):
        ...
        today_stats = super().get_today_stats()
        balance = round((self.limit - today_stats)
                        / CashCalculator.currencies[currency][1], 2)
        if balance > 0:
            return (f'На сегодня осталось '
                    f'{balance} {CashCalculator.currencies[currency][0]}')
        elif balance == 0:
    ...
    def get_week_stats(self):
        """The method get_week_stats just shows
        how much money was spend by user in last 7 days."""
        balanc_for_7_days = super().get_week_stats()
        return f'Потрачено за последние 7 дней: {balanc_for_7_days} рублей.'
